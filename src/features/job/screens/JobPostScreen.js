...
import { sanitizeInput, supabase } from '../../../shared';
import { checkPremiumAccess } from '../../../shared/utils/premiumUtils';
import { useAuth } from '../../auth/hooks/useAuth';

function JobPostScreen({ navigation }) {
  const { user } = useAuth();
  ...
  async function checkAccessPermission() {
    try {
      ...
      console.log('JobPost 권한 체크 결과:', accessResult);
      
      if (!accessResult.isPremium && !accessResult.isAdmin) {
        const message = accessResult.isExpired 
          ? '구독이 만료되었습니다. 채용·협업 공고 업로드는 전문가 멤버십 전용 기능입니다.'
          : '채용·협업 공고 업로드는 전문가 멤버십 전용 기능입니다.';
      ...
      console.error('JobPost 권한 확인 오류:', error);
      navigation.navigate('Upgrade');
    }
  };

  function handleInputChange(field, value) {
    setFormData(function(prev) { return ({
      ...prev,
      [field]: value
    }); });
  };
