...
export function createSafeSupabaseClient() {
  ...
  // 스토리지 인터페이스 강화
  function enhancedStorage(bucket) {
    const original = supabaseClient.storage.from(bucket);
    
    // 안전한 메소드 확장
    return {
      ...original,
      upload: async function(...args) {
        try {
    ...
    auth: {
      ...
      onAuthStateChange: function(callback) {
        try {
          ...
          return { data, unsubscribe: data?.subscription?.unsubscribe || function() {} };
        } catch (error) {
